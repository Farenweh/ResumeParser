...
docx_2pdf.docx_2pdf('docxs/' + demoID + '.docx')

# pdf转jpg功能，用以支持jpg输入，整合了形态学排版方向性检测，可以将潜在的两列排版分割成两张图片，避免OCR导致的语句错位。
pdf_2jpg.pdf_2jpg('pdfs/' + demoID + '.pdf', demo=True)
print('------------------关闭演示窗口以继续流程------------------')

# jpg转txt的ocr功能，通过移动云OCR API实现，整合了置信度筛查和Layout Analyze流程，从而通过版面分析来重排OCR结果，为NLP提供清洁数据
OCR_Analyzed_result = OCRkit.pics_ocr('jpgs/' + demoID, layout_demo=True)
